...
func GetMenuList(c *gin.Context) {
	...
	if err != nil {
		sc.Logger.Ctx(ctx).Error("通过token解析到的userName去数据库中找User失败",
			zap.Error(err),
		)
	...
	for _, role := range roles {
		...
		for _, menu := range role.Menus {
			...
			if err != nil {
				sc.Logger.Ctx(ctx).Error("通过Pid找menu错误", zap.Error(err))
				continue
			}
...
func GetMenuListAll(c *gin.Context) {
	...
	if err != nil {
		sc.Logger.Error("去数据库中拿所有的菜单错误",
			zap.Error(err),
		)
		common.FailWithMessage(fmt.Sprintf("去数据库中拿所有的菜单错误:%v", err.Error()), c)
		return
	}
